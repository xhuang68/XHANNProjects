...
training_set = zeros(32*15, size(features,2));
training_set_norm = zeros(32*15, size(features,2));
testing_set = zeros(637-(32*15),size(features,2));
testing_set_norm = zeros(637-(32*15),size(features,2));
count = 0;
for i = 1:32
    position = find(features(:,1)==i);
    training_set((i-1)*15+1:i*15,:) = features(position(1:15),:);
    remaining = length(position)-15;
    testing_set(count+1:count+remaining,:) = features(position(16:end),:);
